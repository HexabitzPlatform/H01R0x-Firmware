...
uint8_t RGB_LED_State = 0, rgbLedMode = 0, rgbColor = 0, rgbRed = 0, rgbGreen =
		0, rgbBlue = 0;
...
portBASE_TYPE offCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
portBASE_TYPE colorCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
portBASE_TYPE RGBCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
portBASE_TYPE toggleCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
portBASE_TYPE pulseColorCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
portBASE_TYPE pulseRGBCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
portBASE_TYPE sweepCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
portBASE_TYPE dimCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
...
void Module_Init(void) {
	...
	if (RGBledTaskHandle == NULL)
		xTaskCreate(RGBledTask, (const char* ) "RGBledTask",
...
Module_Status Module_MessagingTask(uint16_t code, uint8_t port, uint8_t src,
	...
	case CODE_H01R0_SWEEP:
		period = ((uint32_t) cMessage[port - 1][1 + shift] << 24)
				...
				+ cMessage[port - 1][4 + shift];
		repeat = ((uint32_t) cMessage[port - 1][5 + shift] << 24)
 ...
 */
Module_Status startPWM(uint8_t red, uint8_t green, uint8_t blue,
		uint8_t intensity) {
	...
	/* PWM duty cycle */
	htim3.Instance->CCR2 = ((float) intensity / 100.0f) * ((float) red / 255.0f)
			* period;
	htim3.Instance->CCR3 = ((float) intensity / 100.0f)
			* ((float) green / 255.0f) * period;
	htim3.Instance->CCR4 = ((float) intensity / 100.0f)
			* ((float) blue / 255.0f) * period;
 ...
 */
Module_Status RGB_LED_setRGB(uint8_t red, uint8_t green, uint8_t blue,
		uint8_t intensity) {
	if (intensity == 0)
		return RGB_LED_on(0);
	else if (intensity > 100)
 ...
 */
Module_Status RGB_LED_pulseRGB(uint8_t red, uint8_t green, uint8_t blue,
		uint32_t period, uint32_t dc, int32_t repeat) {
	Module_Status result = H01R0_OK;
 ...
 */
Module_Status RGB_LED_pulseColor(uint8_t color, uint32_t period, uint32_t dc,
		int32_t repeat) {
	Module_Status result = H01R0_OK;
 ...
 */
Module_Status RGB_LED_dim(uint8_t color, uint8_t mode, uint32_t period,
		uint32_t wait, int32_t repeat) {
	if (mode >= 5 && mode <= 12) {
		Module_Status result = H01R0_OK;
...
portBASE_TYPE demoCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	writePxMutex(PcPort, (char*) pcRedMessage, strlen((char*) pcRedMessage), 10,
			10);
	Delay_ms(1000);
	RGB_LED_off();
	RGB_LED_setColor(GREEN, 50);
	writePxMutex(PcPort, (char*) pcGreenMessage, strlen((char*) pcGreenMessage),
			10, 10);
	Delay_ms(1000);
	RGB_LED_off();
	RGB_LED_setColor(BLUE, 50);
	writePxMutex(PcPort, (char*) pcBlueMessage, strlen((char*) pcBlueMessage),
			10, 10);
	Delay_ms(1000);
	RGB_LED_off();
...
portBASE_TYPE onCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	static const int8_t *pcOKMessage =
			(int8_t*) "RGB LED is on at intensity %d%%\r\n";
...
portBASE_TYPE colorCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	static const int8_t *pcOKMessage =
			(int8_t*) "RGB LED color is %s at intensity %d%%\n\r";
...
portBASE_TYPE RGBCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	static const int8_t *pcOKMessage =
			(int8_t*) "RGB LED is (%d, %d, %d) at intensity %d%%\n\r";
	static const int8_t *pcWrongColorMessage =
			(int8_t*) "Wrong color value!\n\r";
	static const int8_t *pcWrongIntensityMessage =
			(int8_t*) "Wrong intensity!\n\r";
...
portBASE_TYPE pulseColorCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	if (result == H01R0_OK) {
		...
		if (repeat == -1)
			...
			sprintf((char*) pcWriteBuffer, (char*) pcMessage, par, period, dc,
					repeat);
	}
...
portBASE_TYPE pulseRGBCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	if (result == H01R0_OK) {
		if (repeat == -1)
			sprintf((char*) pcWriteBuffer, (char*) pcMessageInf, red, green,
					blue, period, dc);
		else
			sprintf((char*) pcWriteBuffer, (char*) pcMessage, red, green, blue,
					period, dc, repeat);
	}
...
portBASE_TYPE sweepCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	else if (!strncmp((char*) pcParameterString1, "fine",
			xParameterStringLength1))
		mode = RGB_SWEEP_FINE;
	...
	if (result == H01R0_OK) {
		...
		if (repeat == -1)
			...
			sprintf((char*) pcWriteBuffer, (char*) pcMessage, par, period,
					repeat);
	}
...
portBASE_TYPE dimCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	if (!strncmp((char*) pcParameterString2, "up", xParameterStringLength2))
		mode = RGB_DIM_UP;
	else if (!strncmp((char*) pcParameterString2, "upwait",
			xParameterStringLength2))
		mode = RGB_DIM_UP_WAIT;
	else if (!strncmp((char*) pcParameterString2, "down",
			xParameterStringLength2))
		mode = RGB_DIM_DOWN;
	else if (!strncmp((char*) pcParameterString2, "downwait",
			xParameterStringLength2))
		mode = RGB_DIM_DOWN_WAIT;
	else if (!strncmp((char*) pcParameterString2, "updown",
			xParameterStringLength2))
		mode = RGB_DIM_UP_DOWN;
	else if (!strncmp((char*) pcParameterString2, "downup",
			xParameterStringLength2))
		mode = RGB_DIM_DOWN_UP;
	else if (!strncmp((char*) pcParameterString2, "updownwait",
			xParameterStringLength2))
		mode = RGB_DIM_UP_DOWN_WAIT;
	else if (!strncmp((char*) pcParameterString2, "downupwait",
			xParameterStringLength2))
	...
	if (result == H01R0_OK) {
		...
		if (repeat == -1)
			sprintf((char*) pcWriteBuffer, (char*) pcMessageInf, par1, period,
					wait, par2);
		else
			sprintf((char*) pcWriteBuffer, (char*) pcMessage, par1, period,
					wait, par2, repeat);
	}
